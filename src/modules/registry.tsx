...
export interface ModuleInspectorProps<TConfig> extends ModuleViewProps<TConfig> { onChange: (config: TConfig) => void; }
...
interface TypedModuleDefinition<TSchema extends ZodTypeAny> {
  ...
  createDefaultConfig: () => output<TSchema>; ConfigSchema: TSchema;
...
export interface ModuleDefinition {
  type: string; title: string; description: string; icon: LucideIcon; version: number;
  createDefaultConfig: () => unknown; ConfigSchema: ZodTypeAny;
...
function defineModule<TSchema extends ZodTypeAny>(definition: TypedModuleDefinition<TSchema>): ModuleDefinition {
  ...
  return { ...definition, PreviewComponent: ({ config }) => <TypedPreview config={definition.ConfigSchema.parse(config)} />, InspectorComponent: ({ config, onChange }) => <TypedInspector config={definition.ConfigSchema.parse(config)} onChange={(next) => onChange(next)} /> };
}

export const moduleRegistry = {
  loyalty_passport: defineModule({
    type: 'loyalty_passport', title: 'Loyalty Passport', description: 'Визиты, прогресс и награда', icon: CreditCard, version: 1,
    createDefaultConfig: createLoyaltyConfig, ConfigSchema: LoyaltyPassportConfigSchema, PreviewComponent: LoyaltyPassportPreview, InspectorComponent: LoyaltyPassportInspector,
    ai: { purpose: 'Программа лояльности по визитам или покупкам', keywords: ['бонус', 'лояльность', 'каждый шестой бесплатно', 'визиты', 'штампы'], examples: ['Каждый 6 кофе бесплатно', 'Добавь программу лояльности'] },
  }),
  offers_placeholder: defineModule({
    type: 'offers_placeholder', title: 'Offers', description: 'Foundation для предложений', icon: Tags, version: 1,
    createDefaultConfig: createOffersConfig, ConfigSchema: OffersConfigSchema, PreviewComponent: OffersPreview, InspectorComponent: OffersInspector,
    ai: { purpose: 'Акции и специальные предложения для клиентов', keywords: ['акции', 'предложения', 'скидки', 'промо'], examples: ['Добавь акции', 'Убери предложения'] },
  }),
