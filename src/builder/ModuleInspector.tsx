...
export function ModuleInspector({ module, projectId, onChange, onReset }: {
  ...
  return <Card className="inspector"><div className="section-heading"><div><span className="eyebrow">Инспектор</span><h2>{definition.title}</h2></div></div><ModuleErrorBoundary resetKey={JSON.stringify(parsed.data)} onReset={onReset}><definition.InspectorComponent config={parsed.data} onChange={onChange} projectId={projectId} moduleId={module.id} /></ModuleErrorBoundary></Card>;
}
