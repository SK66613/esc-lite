...
describe('Passport media client',()=>{
  ...
  it.each([
    ['MEDIA_FILE_TOO_LARGE','Файл слишком большой'],['MEDIA_PROJECT_QUOTA','В этом проекте достигнут лимит'],['MEDIA_USER_QUOTA','Достигнут лимит'],['MEDIA_RATE_LIMIT','Слишком много загрузок'],['MEDIA_INVALID_TYPE','Неподдерживаемый формат'],['MEDIA_NOT_CONFIGURED','Хранилище временно недоступно'],
  ])('maps %s to a friendly message',async(code,message)=>expect(run(json({code},code==='MEDIA_INVALID_TYPE'?415:429))).rejects.toThrow(message));
});
