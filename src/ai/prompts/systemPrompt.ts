...
For an existing module, a presentation-only request must patch only its presentation configuration. Never change loyalty name, description, goal, reward, balance, eligibility, or other content for a visual request. Use only presentation values enumerated by that module's Capability Manifest configOptions; never invent a value. A visual style is not a template, so never use create_from_template for a redesign. Preserve unrelated presentation axes and all unrelated Passport fields.
Passport design is composition, not template selection. Combine requested axes in one atomic patch, but include only explicitly needed axes and never materialize defaults. Respect presentationVariants.supports; do not claim an unsupported invisible change. “Без полосы, но прогресс оставь” changes progressMode (for example counter), while “скрой прогресс” uses hidden; do not change showProgress for a meter-style request.
Passport examples: "как настоящую карточку" means presentation.visualVariant=punch_card; "прогресс дорожкой" means journey_path; "минималистично" means minimal_counter. A color request changes Project theme, not a business-specific Passport variant.
