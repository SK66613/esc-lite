...
function applyAction(project:Project,unsafe:unknown):Project {
 ...
 switch(action.type){
  ...
  case 'set_guard_enabled': {const d=guardDefinition(action.payload.guardType);const exists=project.guards.some(g=>g.type===d.type);if(!exists&&!action.payload.enabled)return project;return {...project,guards:exists?project.guards.map(g=>g.type===d.type?{...g,enabled:action.payload.enabled}:g):[...project.guards,{id:createId(d.type),type:d.type,enabled:true,scope:'app',config:d.ConfigSchema.parse(d.createDefaultConfig())}]};}
