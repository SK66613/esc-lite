...
const AIMetadataSchema = z.object({
  ...
  configStructures:z.record(z.string().min(1).max(200),z.object({kind:z.literal('position_map'),description:z.string().max(500),keyRange:z.object({min:z.number().int().min(0).max(1000),max:z.number().int().min(1).max(1000)}).strict(),fields:z.record(z.string().min(1).max(100),z.object({type:z.enum(['string','object']),description:z.string().max(500).optional(),nullable:z.boolean().optional(),maxLength:z.number().int().positive().max(10000).optional(),values:z.array(z.union([z.string().max(200),z.number(),z.boolean()])).max(100).optional(),fields:z.record(z.string().min(1).max(100),z.object({type:z.literal('string'),values:z.array(z.string().min(1).max(200)).min(1).max(100)}).strict()).refine(fields=>Object.keys(fields).length<=10).optional()}).strict()).refine(fields=>Object.keys(fields).length<=20)}).strict()).refine(structures=>Object.keys(structures).length<=20).optional(),
  coverAssets:z.array(z.object({id:z.string().min(1).max(80),title:z.string().min(1).max(120),category:z.string().min(1).max(60),keywords:z.array(z.string().min(1).max(60)).max(8)}).strict()).max(40).optional(),
}).strict();
