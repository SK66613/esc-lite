...
export type Route='home'|'builder'|'preview'|'settings'|'tool'|'access';
...
export const useEditorStore=create<EditorStore>(set=>({route:'home',previewSource:'draft',device:'phone',setRoute:route=>set({route}),selectModule:selectedModuleId=>set({selectedModuleId}),setSheet:sheet=>set({sheet}),setPreviewSource:previewSource=>set({previewSource}),setDevice:device=>set({device})}));
