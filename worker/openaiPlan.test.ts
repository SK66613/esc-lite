...
describe('authoritative AI media policy regressions',()=>{
 const action=(patch:Record<string,unknown>)=>({type:'patch_module_config',payload:{moduleType:'loyalty_passport',patch}});
 it('rejects media even when client manifest advertises it',async()=>{const req=structuredClone(request('Поставь фото')) as any;const cover=req.capabilities.modules.find((m:any)=>m.type==='loyalty_passport').ai.configStructures.stampContent.fields.cover;cover.fields.source.values.push('media');cover.fields.mediaId={type:'string',values:['invented']};const mediaId='m1_abcdefghijklmnop_qrstuvwxyzABCDEF_123e4567-e89b-42d3-a456-426614174000';await expect(createOpenAIPlan(req,{apiKey:'secret',fetchImpl:async()=>responseWithPlan(modelPlan([action({stampContent:{'2':{cover:{source:'media',mediaId}}}})]))})).rejects.toMatchObject({code:'AI_INVALID_PLAN'});});
 it('accepts catalog and null covers',async()=>{for(const cover of [{source:'catalog',assetId:'gift-box'},null])await expect(createOpenAIPlan(request(),{apiKey:'secret',fetchImpl:async()=>responseWithPlan(modelPlan([action({stampContent:{'2':{cover}}})]))})).resolves.toBeDefined();});
 it('leaves an existing media cover untouched for presentation-only plans',async()=>{const req=request('Сделай как путь');const module=req.project.modules.find(m=>m.type==='loyalty_passport')!;(module.config as any).stampContent={'2':{title:'Диагностика',cover:{source:'media',mediaId:'m1_abcdefghijklmnop_qrstuvwxyzABCDEF_123e4567-e89b-42d3-a456-426614174000'}}};const plan=await createOpenAIPlan(req,{apiKey:'secret',fetchImpl:async()=>responseWithPlan(modelPlan([action({presentation:{visualVariant:'journey_path'}})]))});expect(plan.actions[0]).toEqual(action({presentation:{visualVariant:'journey_path'}}));expect((module.config as any).stampContent['2'].cover.source).toBe('media');});
});
