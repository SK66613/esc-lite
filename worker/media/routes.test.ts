...
describe('public Passport media GET',()=>{
  ...
  it('maps R2 get failure safely',async()=>{const bucket=new FakeBucket();bucket.fail='get';const response=await handleMediaGet(mediaId,env(bucket));expect(response.status).toBe(503);expect(await bodyCode(response)).toBe('MEDIA_STORAGE_GET')});
});
